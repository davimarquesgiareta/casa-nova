...
export function ShowsGrid() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast({
        title: "Sucesso",
        description: `Show ${editingShow ? "atualizado" : "criado"}!`,
      });
  ...
  const handleEdit = (show: Show) => {
    ...
    setFormData({
      name: show.name,
      venue: show.venue || "",
      event_date: show.event_date ? show.event_date.split("T")[0] : "",
  ...
  const executeDelete = async () => {
    ...
    try {
      ...
      toast({
        title: "Sucesso",
        description: `Show "${showToDelete.name}" excluído!`,
      });
      fetchShows();
    } catch (error) {
      toast({
  ...
  const handleAddSongToShow = async (song: Song, showId: string) => {
    ...
    if (selectedShow.songs.find((s) => s.id === song.id)) {
      toast({
        title: "Aviso",
        description: "Essa música já está no repertório.",
      });
      return;
    }
  ...
  const handleRemoveSongFromShow = async (songId: string) => {
    ...
    try {
      const response = await fetch(
        `/api/shows/${selectedShow.id}/songs/${songId}`,
        { method: "DELETE" }
      );
      if (!response.ok) {
  ...
  const handleDropOnCard = (e: React.DragEvent, showId: string) => {
    e.preventDefault();
    setDragOverShow(null);
    const songData = JSON.parse(
      e.dataTransfer.getData("application/json")
    ) as Song;
    fetch(`/api/shows/${showId}/songs`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      ...
      if (res.ok) {
        toast({
          ...
          description: `"${songData.title}" adicionada a "${
  ...
  const resetForm = () => {
    setFormData({ name: "", venue: "", event_date: "", show_time: "" });
  };
      ...
      fetch(`/api/shows/${selectedShow.id}/songs/reorder`, {
        ...
        .then((res) => {
          if (res.ok) {
            toast({
              title: "Sucesso",
              description: "Ordem do repertório salva!",
            });
          } else {
  ...
  const handleSendToWhatsApp = () => {
    ...
    if (!whatsappNumber.trim()) {
      toast({
        title: "Erro",
        description: "Por favor, insira ou selecione um número.",
        variant: "destructive",
      });
      return;
    }
  ...
  // MUDANÇA: Função de impressão refinada e correta
  const handlePrint = () => {
    if (!selectedShow) return;

    const formattedDate = selectedShow.event_date
      ? new Intl.DateTimeFormat("pt-BR", { timeZone: "UTC" }).format(
          new Date(selectedShow.event_date)
        )
      : null;
    ...
    let content = `
      <style>
        ...
        p { margin: 0.25rem 0; font-size: 1.125rem; }
        .details { margin-bottom: 1.5rem; }
        .song-list { margin-top: 1.5rem; }
        .song-list p { font-size: 1rem; margin-bottom: 0.5rem; }
      </style>
    ...
    if (selectedShow.venue) {
      content += `<p>${selectedShow.venue}</p>`;
    }
    if (formattedDate || selectedShow.show_time) {
      content += `<p>
        ${formattedDate ? `Data: ${formattedDate}` : ""}
        ${
          formattedDate && selectedShow.show_time
            ? "&nbsp;&nbsp;&nbsp;|&nbsp;&nbsp;&nbsp;"
            : ""
        }
        ${selectedShow.show_time ? `Horário: ${selectedShow.show_time}` : ""}
      </p>`;
    }

    content += '</div><hr/><div class="song-list">';

    selectedShow.songs.forEach((song, index) => {
      content += `<p>${index + 1}. ${song.title} - ${song.artist}</p>`;
    });
    ...
    if (printWindow) {
      printWindow.document.write(`
        <html>
          <head><title>Imprimir Repertório - ${selectedShow.name}</title></head>
          <body>${content}</body>
        </html>
      `);
      printWindow.document.close();
      printWindow.focus();
      // Um pequeno timeout para garantir que o conteúdo foi renderizado antes de imprimir
      setTimeout(() => {
        printWindow.print();
        printWindow.close();
      }, 250);
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row gap-4 items-center">
        ...
        <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
          ...
          <DialogContent className="sm:max-w-md">
            <DialogHeader>
              <DialogTitle>
                {editingShow ? "Editar Show" : "Criar Novo Show"}
              </DialogTitle>
            </DialogHeader>
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="grid gap-2">
                  <Label htmlFor="show_time">Horário (opcional)</Label>
                  <Input
                    id="show_time"
                    type="time"
      ...
      {filteredShows.length === 0 ? (
        <Card>
          <CardContent className="text-center py-12">
            ...
            <h3 className="text-lg font-semibold">Nenhum show criado</h3>
          </CardContent>
        </Card>
      ) : (
          ...
          {filteredShows.map((show) => (
            <Card
              ...
              className={`hover:shadow-md transition-all ${
                dragOverShow === show.id ? "ring-2 ring-primary" : ""
              }`}
              onDragOver={(e) => handleDragOver(e, show.id)}
              onDragLeave={handleDragLeave}
              onDrop={(e) => handleDropOnCard(e, show.id)}
            >
              <CardHeader className="pb-3">
                <div className="flex items-start justify-between">
                  <CardTitle className="text-lg flex items-center gap-2">
                    <Folder className="w-5 h-5 text-primary flex-shrink-0" />
                    <span className="break-words">{show.name}</span>
                  </CardTitle>
                  <div className="flex gap-1">
                    ...
                    <Button
                      ...
                      className="text-destructive hover:bg-[#021c25] hover:text-white rounded-full"
                    >
      ...
      <Dialog
        open={isShowDetailOpen}
        onOpenChange={(isOpen) => !isOpen && handleCloseDetailModal()}
      >
              ...
              <div>
                ...
                <div className="flex items-center gap-4 text-sm text-muted-foreground pt-1">
                  <div className="flex items-center gap-1.5">
                    ...
                    <span>{selectedShow?.songs.length || 0} músicas</span>
                  </div>
                  <div className="flex items-center gap-1.5">
                    <Clock className="w-4 h-4" />
                    <span>{calculateTotalDuration(selectedShow?.songs)}</span>
                  </div>
                </div>
              </div>
              {selectedShow && selectedShow.songs.length > 0 && (
                <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2">
                  ...
                  <Button
                    ...
                    className="bg-[#25D366] text-white hover:bg-[#25D366] hover:opacity-90 w-full sm:w-auto"
                  >
          ...
          {selectedShow && (
            <div className="grid lg:grid-cols-2 gap-6 flex-1 overflow-hidden">
              <div className="flex flex-col min-h-0">
                ...
                >
                  <SortableContext
                    items={selectedShow.songs.map((s) => s.id)}
                    strategy={verticalListSortingStrategy}
                  >
                    <div
                      className="flex-1 overflow-y-auto space-y-2 pr-2 border-2 border-dashed border-transparent rounded-lg"
                      onDragOver={(e) => e.preventDefault()}
                      onDrop={handleDropOnSetlist}
                    >
                      {selectedShow.songs.length === 0 ? (
                        <div className="text-center text-muted-foreground py-10">
                          Arraste músicas ou use o botão "Adicionar".
                        </div>
                      ) : (
                        selectedShow.songs.map((song, index) => (
                          <SortableSongItem
                            ...
                            song={song}
                            index={index}
                            onRemove={handleRemoveSongFromShow}
                          />
                        ))
                      )}
                    </div>
              ...
              {!isMobile && (
                <div className="flex flex-col min-h-0">
                  ...
                  <MusicLibraryForShowComponent
                    onAddSong={(song) =>
                      handleAddSongToShow(song, selectedShow.id)
                    }
                    showSongs={selectedShow.songs}
                  />
                </div>
              )}
            </div>
          ...
          <div className="space-y-4 py-2">
              ...
              <Select onValueChange={(value) => setWhatsappNumber(value)}>
                ...
                <SelectContent>
                  {PRESET_CONTACTS.map((contact) => (
                    <SelectItem key={contact.name} value={contact.number}>
                      {contact.name}
                    </SelectItem>
                  ))}
                </SelectContent>
            ...
            <div className="grid gap-2">
              <Label htmlFor="whatsapp-number">
                ou Digite o Número com DDD
              </Label>
              <Input
                id="whatsapp-number"
                placeholder="Ex: 19989305698"
                type="tel"
            ...
            <div className="bg-muted p-3 rounded-lg max-h-40 overflow-y-auto">
              ...
              <div className="text-xs text-muted-foreground space-y-1 whitespace-pre-wrap">
                {`*${
                  ...
                  .slice(0, 3)
                  .map((s, i) => `${i + 1}. ${s.title} - _${s.artist}_`)
                  .join("\n")}${
      ...
      <AlertDialog
        open={!!showToClone}
        onOpenChange={(isOpen) => !isOpen && setShowToClone(null)}
      >
        <AlertDialogContent>
          <AlertDialogHeader>
