...
export function ShowsGrid() {
  ...
  // MUDANÇA 1: Criamos uma função para fechar o modal de detalhes
  // que também garante que a lista principal está atualizada.
  const handleCloseDetailModal = () => {
    setIsShowDetailOpen(false);
    setSelectedShow(null);
    // Apenas quando fechamos o modal, nós re-sincronizamos a lista principal
    fetchShows();
  }
  ...
  // MUDANÇA 2: Simplificamos a função para NÃO chamar fetchShows() aqui.
  // Isso evita a re-renderização da tela inteira.
  const handleAddSongToShow = async (song: Song, showId: string) => {
      ...
      try {
          ...
          // Sucesso! Não fazemos mais nada aqui. A UI já está atualizada.
      } catch (error: any) {
  ...
  const handleRemoveSongFromShow = async (songId: string) => {
    ...
    try {
        ...
        // Também não chamamos fetchShows() aqui.
    } catch(error) {
  ...
  const handleDropOnCard = (e: React.DragEvent, showId: string) => {
    ...
    fetch(`/api/shows/${showId}/songs`, {
        ...
        if(res.ok) {
            toast({ title: "Música adicionada!", description: `"${songData.title}" adicionada a "${shows.find(s=>s.id===showId)?.name}".`});
            fetchShows();
        } else {
            toast({ title: "Erro", description: "Não foi possível adicionar a música.", variant: "destructive"});
        }
    });
  };
  ...
  return (
    <div className="space-y-6">
      ...
      {/* MUDANÇA 3: O `onOpenChange` agora usa nossa nova função `handleCloseDetailModal` */}
