...
// MUDANÇA 1: Adicionamos a interface de props
...
// MUDANÇA 2: O componente agora aceita a prop 'onDataChange'
export function ShowsGrid({ onDataChange }: ShowsGridProps) {
  ...
  const fetchShows = async () => { setLoading(true); try { const response = await fetch('/api/shows'); if (!response.ok) throw new Error('Erro ao buscar shows'); const data = await response.json(); setShows(data); } catch (error) { toast({ title: "Erro", description: "Não foi possível carregar os shows.", variant: "destructive" }); } finally { setLoading(false); } };
  ...
  const executeClone = async () => {
    ...
    try {
      const response = await fetch(`/api/shows/${showToClone.id}/clone`, { method: 'POST', });
      if (!response.ok) { throw new Error("Não foi possível clonar o repertório."); }
      toast({ title: "Sucesso!", description: `Repertório "${showToClone.name}" clonado.`, });
      await fetchShows();
      onDataChange();
    } catch (error: any) {
      toast({ title: "Erro", description: error.message, variant: "destructive", });
            ...
            <div className="flex justify-between items-start">
              ...
              {selectedShow && selectedShow.songs.length > 0 && (
                <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2">
                  ...
                  <div className="mr-5">
                  <Button size="sm" onClick={() => setIsWhatsAppModalOpen(true)} className="bg-[#25D366] text-white hover:bg-[#25D366] hover:opacity-90 w-full sm:w-auto">
